...
module.exports = async(req, res, next)=>{
    ...
    if(!localStorage.getItem('token')) return res.status(401).json({message: "token not found (Backend Error)"}); // for development mode
    ...
    try {
        ...
        let decoded = jwt.verify(localStorage.getItem('token'), process.env.JWT_SECRET_KEY); // for development mode
