...
router.post("/addproduct", isLoggedIn,async(req, res)=>{
    ...
    try {
        ...
        if(user.role !== "admin") return res.status(403).json({message: "You are not authorized to add a product"});
...
router.post('/updateProduct/:id', isLoggedIn, async(req, res)=>{
    ...
    try {
        ...
        if(user.role !== "admin") return res.status(403).json({message: "You are not authorized to update a product"});
...
router.get('/getProductByGenderType/:genderType', async(req, res)=>{
    try {
        const products = await productModel.find({genderType: req.params.genderType});
        return res.status(200).json({
            message: "Products fetched successfully",
            products
        })
